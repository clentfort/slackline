...
export class MessageManager extends SlackComponent {
  ...
  private async readVisibleSnapshots(): Promise<SlackMessageSnapshot[]> {
    return this.page.locator('[data-qa="message_pane"] [data-qa="message_container"]').evaluateAll((nodes, helpers) => {
  ...
  private static dedupe<T extends SlackMessage>(messages: T[]): T[] {
    ...
    for (const message of messages) {
      const key = [
        message.user ?? '',
        typeof message.timestampUnix === 'number' ? message.timestampUnix.toString() : message.timestampLabel ?? '',
        message.text,
      ].join('|')
  ...
  private async sendComposerMessage(composer: Locator): Promise<void> {
    ...
    const sendButton = hasScopedSendButton ? scopedSendButton : this.page.locator('[data-qa="texty_send_button"]').first()
  ...
  private async locateComposer(): Promise<Locator> {
    ...
    const selectors = ['[data-qa="texty_input"][data-input-metric-boundary="composer"][contenteditable="true"]']
  ...
  private async pickWidestVisibleComposer(selector: string, waitTimeoutMs: number): Promise<Locator | undefined> {
  ...
  private async waitForPosted(
    ...
    while (Date.now() < deadline) {
      ...
      for (const latest of latestMessages) {
        ...
        const isFreshEnough =
          typeof latest.timestampUnix !== 'number' || latest.timestampUnix + 1 >= previous.sentAtUnix
